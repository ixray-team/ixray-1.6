...
bool CUIActorMenu::CanSetItemToList(PIItem item, CUIDragDropListEx* l, u16& ret_slot)
{
	...
	if (item_slot == INV_SLOT_3 && l == m_pInvList[INV_SLOT_2] && EngineExternal().CallOfPripyatMode())
	{
		ret_slot	= INV_SLOT_2;
		return		true;
	}

	if (item_slot == INV_SLOT_2&& l == m_pInvList[INV_SLOT_3] && EngineExternal().CallOfPripyatMode())
	{
		ret_slot	= INV_SLOT_3;
		return		true;
	}
