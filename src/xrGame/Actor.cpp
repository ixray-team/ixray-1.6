...
CActor::CActor() : CEntityAlive(), current_ik_cam_shift(0)
{
	...
	m_fControlledActorSpeedKoef = 0.0f;
	m_fControllerPrepareTime = 0.0f;
	m_fActorShockTime = 0.0f;
	m_fActorCameraLanding2Time = 0.0f;
	m_fActorCameraSpeedPow = 0.0f;
	m_fDefaultActorCameraSpeed = 0.0f;
	m_fActorCameraLandingOffset = 0.0f;
	m_fActorCameraLandingSpeedFactor = 0.0f;
	m_fActorCameraLandingSpeedPowFactor = 0.0f;
	m_fActorCameraFinishLandingTime = 0.0f;
	m_fLookOutSpeed = 0.0f;
	m_fLookOutAmplK = 0.0f;
	m_fLookOutSpeedAmplDXPow = 0.0f;

	m_pActorNightVisionDisableAnimatorName = nullptr;
	m_pActorNightVisionEnableAnimatorName = nullptr;
	m_pActorNightVisionHUDDisableAnimationName = nullptr;
	m_pActorNightVisionHUDEnableAnimationName = nullptr;
	m_pActorHeadLampDisableAnimatorSectionName = nullptr;
...
void CActor::Load(LPCSTR section)
{
	...
	*/
	...
	m_fControllerPrepareTime = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "controller_prepare_time", 3.f);

	m_fControlledActorSpeedKoef = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "controlled_actor_speed_koef", 1.0f);

	m_fActorShockTime = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "actor_shock_time", 10.0f);
	...
	m_fActorCameraSpeedPow = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "actor_camera_speed_pow", 1.f);
	m_fDefaultActorCameraSpeed = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "default_actor_camera_speed", 10.f);
	m_fActorCameraLandingOffset = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "actor_camera_landing_offset", 0.f);
	m_fActorCameraLandingSpeedFactor = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "actor_camera_landing_speed_factor", 1.f);
	m_fActorCameraLandingSpeedPowFactor = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "actor_camera_landing_speed_pow_factor", 1.f);
	m_fActorCameraLanding2Offset = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "actor_camera_landing2_offset", 0.f);
	m_fActorCameraLanding2SpeedFactor = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "actor_camera_landing2_speed_factor", 1.f);
	m_fActorCameraLanding2SpeedPowFactor = READ_IF_EXISTS(pSettings, r_float, "gunslinger_base", "actor_camera_landing2_speed_pow_factor", 1.f);
...
bool CActor::IsControllerPreparing() const
{
	if (IsPsiBlocked() && !IsPsiBlockFailed())
		...
		return Device.GetTimeDeltaSafe(_controller_preparing_starttime) < floor(m_fControllerPrepareTime * 1000.f) + 1000.f;
}
...
float CActor::GetCurrentSuicideWalkKoef() const
{
	if (IsActorControlled() || IsActorSuicideNow() || IsActorPlanningSuicide() || IsControllerPreparing())
		return m_fControlledActorSpeedKoef;
...
void CActor::UpdateSuicide(u32 dt)
{
	...
	if (_controlled_time_remains == 0 && _lastshot_done_time == 0 && !_death_action_started)
	{
		...
		if (last_contr_time > 0)
			SetHandsJitterTime(floor(m_fActorShockTime * 1000.0f));
