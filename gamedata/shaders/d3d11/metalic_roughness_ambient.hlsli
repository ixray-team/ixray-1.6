...
{
    float4 R = Roughness * float4(-1, -0.0275, -0.572, 0.022) + float4(1.0f, 0.0425f, 1.04f, -0.04f);
...
float3 CompureDiffuseIrradance(float3 N, float Hemi)
{
    ...
    float3 SampleLast = pow(env_s0.SampleLevel(smp_rtlinear, LightDirection, 0.0f), 2.2f);
    float3 SampleNext = pow(env_s1.SampleLevel(smp_rtlinear, LightDirection, 0.0f), 2.2f);

    float3 Irradance = L_hemi_color.xyz * lerp(SampleLast, SampleNext, L_hemi_color.w);
    Irradance = lerp(Irradance, dot(Irradance, LUMINANCE_VECTOR), 0.5f);
...
float3 CompureSpecularIrradance(float3 R, float Hemi, float Roughness)
{
    ...
    float3 SampleLastD = pow(env_s0.SampleLevel(smp_rtlinear, LightDirection, 0.0f).xyz, 2.2f);
    float3 SampleNextD = pow(env_s1.SampleLevel(smp_rtlinear, LightDirection, 0.0f).xyz, 2.2f);
#endif
    ...
    float3 SampleLast = pow(sky_s0.SampleLevel(smp_rtlinear, LightDirection, Lod), 2.2f);
    float3 SampleNext = pow(sky_s1.SampleLevel(smp_rtlinear, LightDirection, Lod), 2.2f);
